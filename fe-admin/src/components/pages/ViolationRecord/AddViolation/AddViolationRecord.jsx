...
const AddViolationRecord = () => {
  ...
  const validateFields = () => {
    ...
    if (!startDate) errors.thoiGianLap = "Thời gian lập không được để trống.";
    if (!resolveDate)
  ...
  const handleDateChange = (date, type) => {
    if (type === "thoiGianLap") {
      setStartDate(new Date(date)); // Cập nhật trạng thái với ngày mới
      // console.log("Thời gian lập:", date);
    } else if (type === "thoiGianViPham") {
      setViolationDate(new Date(date)); // Cập nhật trạng thái với ngày vi phạm
      // console.log("Thời gian vi phạm:", date);
    } else if (type === "thoiGianGiaiQuyet") {
      setResolveDate(new Date(date));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const thoiGianLapMoi = startDate ? startDate.toISOString() : null;
    const thoiGianViPhamMoi = violationDate
      ? violationDate.toISOString()
      : null;
    const thoiGianGiaiQuyetMoi = resolveDate ? resolveDate.toISOString() : null;
    ...
    try {
      ...
      toast.error(
        error.response.data || "Lỗi tạo biên bản vi phạm, hãy thử lại."
      );
    }
  };
  ...
  return (
    <div id="box" className="container-xl bg-white ps-5 pe-lg-5 pt-4">
      <h1 className="text-center fw-700 mb-3 mt-3">BIÊN BẢN VI PHẠM</h1>
          ...
          <div className="row">
            ...
            <div className="col-md-4">
              ...
              <div>
                <DatePicker
                  selected={startDate}
                  onChange={(date) => handleDateChange(date, "thoiGianLap")}
                  showTimeSelect
                  timeFormat="HH:mm"
            ...
            <div className="col-md-4">
              <div className="mb-3">
                ...
                <DatePicker
                  ...
                  onChange={(date) => handleDateChange(date, "thoiGianViPham")}
                  showTimeSelect
                  timeFormat="HH:mm"
                  timeIntervals={15}
                  dateFormat="Pp"
                ...
                <Select
                  ...
                  value={
                    formattedOptions
                      ...
                      .find(
                        (option) => option.value === mainData.viPhamDieuKhoan
                      ) || null
                  }
