...
class AuthService {
  ...
  resetPassword({ token, newPassword, verificationCode }) {
    return axios.post(API_URL + 'reset-password', { token, newPassword, verificationCode })
        .then(response => {
            console.log('Phản hồi từ server:', response);
            console.log('Token:', token);
            console.log('Mật khẩu mới:', newPassword);
            console.log('Mã xác nhận:', verificationCode);
            return { success: true, message: response.data.message || "Mật khẩu đã được đặt lại thành công!" };
        })
        .catch(error => {
